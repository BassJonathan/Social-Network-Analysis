...
rnc[[1]]
net2 %>% as_tbl_graph() %>% activate(nodes) %>% mutate(role <- merge(x = net2 , y = rnc[[1]][ , c("name", "module", "role")], by = "name", all = TRUE)) -> net3
...
net3
ggnet2(net3, mode = "adj", node.color = "Status", palette = "Set1", node.size = "module", shape = "role")
```
